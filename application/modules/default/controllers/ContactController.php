 ...
 * Time: 9:11 PM
 */
class ContactController extends Application_Controller_FrontEnd_Default
{
    ...
    public function listingAction()
    {
        ...
        $this->setMetaData(
            'Tin tìm phòng tại HCM | Xome.vn',
            $this->getTranslateValue('common_keywords'),
            'Bạn cần tim phòng gần các trường đại học, khu vực gần chợ, thuận tiện đi lại... Hãy đăng tin xome.vn để nhanh chóng tìm dược phòng!'
        );
    }
    ...
    {
        $url = 'http://xome.vn/upload/staticmap.png';

        $this->setMetaImage($url);
        $this->setMetaData(
            'Đăng tin tìm phòng tại xome.vn',
            $this->getTranslateValue('common_keywords'),
            'Bạn cần tim phòng gần các trường đại học, khu vực gần chợ, thuận tiện đi lại... Hãy đăng tin xome.vn để nhanh chóng tìm dược phòng!'
        );
    }
        ...
        if ($encode) {
            ...
            if ($dataInfo) {
              ...
              //  $this->view->assign('encodeStringMap', $encodeStringMap);
                ...
                $this->setMetaImage($url);
                $this->setMetaData(
                    'Cần thuê phòng gần khu vực '. $dataInfo[DbTable_Contact::COL_CONTACT_ADDRESS],
                    $this->getTranslateValue('common_keywords'),
